<template>
    <div class="widget">
        <div class="widget-header">
            ...
            <div v-if="walletAddressComputed && !isCreatorPage" @click="showWindow" class="widget-button"><span class="material-icons">add</span></div>
        </div>
        ...
        <div class="app-window" @click.stop>
            ...
            <div class="widget-content widget-scrollable-window">
                ...
                <div v-if="reviewStatus === 'error'">
                    ...
                    <div>
                        <div tabindex="0" @click="closeWindow">Close</div>
                    </div>
                </div>
...
export default {
    ...
    setup() {
        ...
        const creatorId = ref(route.params.id);
        ...
        const fetchCreatorReviews = async () => {
            try {
                const response = await fetch(
                    config.api + '/getCreatorReviews/' + route.params.id
                );
        ...
        async function getUserReview() {
            try {
                const response = await fetch(`${config.api}/getUserReviewForCreator/${walletAddress.value}/${creatorId.value}`);
                const data = await response.json();
        ...
        async function sendReview() {
            ...
            if (isCreatorPage.value) {
                alert("You can't write a review for yourself");
                isSubmitting.value = false;
                closeWindow();
                return;
            }
            ...
            const success = await sendReviewForCreator(userReviewText);
            if (success) {
        ...
        async function sendReviewForCreator(userReviewText) {
            try {
                const authToken = localStorage.getItem('auth_token');
                const response = await fetch(`${config.api}/sendUserReviewForCreator`, {
                    method: 'POST',
                    headers: {
                        'Content-Type': 'application/json',
        ...
        const isCreatorPage = computed(() => {
            ...
            return (
                walletAddressComputed.value &&
                storedWalletAddress &&
                (route.params.id === walletAddressComputed.value || route.params.id === storedWalletAddress)
            );
        });
