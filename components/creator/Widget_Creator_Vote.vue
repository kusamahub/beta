...
export default {
    ...
    setup() {
        ...
        async function getUserRating() {
            try {
                const response = await fetch(`${config.api}/getUserRatingForCreator/${walletAddress.value}/${creatorId.value}`);
        ...
        async function vote(rating) {
            ...
            if (isCreatorPage.value) {
                alert("You can't set a rating for your self");
                return;
            }
        ...
        async function updateUserRating(rating) {
            try {
                ...
                const response = await fetch(`${config.api}/updateUserRatingForCreator`, {
                    method: 'POST',
                    headers: {
        ...
        // Check if creator page belongs to creator
        const isCreatorPage = computed(() => {
            const storedWalletAddress = localStorage.getItem('user_wallet');
            return (
                walletAddress.value &&
                storedWalletAddress &&
                (route.params.id === walletAddress.value || route.params.id === storedWalletAddress)
            );
        });
        ...
        async function signMessage() {
            try {
                ...
                if (!signedResult) { // Check if the signature is present
                    return false;
                }
