<!-- Widget: User Rating for Colletion -->
<!-- Users rating for current collection -->
<!-- Wiki:  -->
<template>
    <div class="widget">
...
export default {
    name: 'Widget_Collection_User_Rating',
    ...
    setup() {
        ...
        onMounted(async () => {
            try {
                const responseThumbUp = await fetch(config.api + '/getCollectionRating/' + route.params.id + '/vote_up');
                const dataThumbUp = await responseThumbUp.json();
                ratingThumbUp.value = dataThumbUp.thumbs_up || 0;

                const responseThumbDown = await fetch(config.api + '/getCollectionRating/' + route.params.id + '/vote_down');
